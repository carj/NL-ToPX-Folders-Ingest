...
if __name__ == '__main__':
    ...
    for archief in archief_folders:
        ...
        if len(entities) == 0:
            ...
            folder = entities.pop()
            archief_parent = folder.reference
        dossier_folders = [f.name for f in os.scandir(archief_folder) if f.is_dir()]
        for dossier in dossier_folders:
            logger.info(f"Found {dossier} Dossier Folder")
            dossier_folder = os.path.join(os.path.join(archief_folder, dossier))
            ...
            entities = entity.identifier("identificatiekenmerk", identifier)
            if len(entities) == 0:
                folder = entity.create_folder(dossier, dossier, security_tag, archief_parent)
                entity.add_identifier(folder, "identificatiekenmerk", identifier)
            ...
            for batch in chunks(record_folders, group_size):
                ...
                if len(batches) > 0:
                    ...
                    logger.info(
                        f"Created submission with {group_size} folders and total size of {package_size_MB}MB")

                    if package_size_MB > 32:
